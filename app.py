...
st.markdown("""
    ...
    .alert-box {
        ...
        color: #e5e9f0; padding: 15px; border-radius: 10px; font-size: 0.95rem; margin-bottom: 25px;
    }
...
with tab1:
    ...
    news_clip = st.text_area("🔍 欲分析的教育新聞文本：", height=150, placeholder="貼上新聞或文章片段，AI 將協助您轉化為考試考點...")
    if st.button("🎯 執行深度考點轉化"):
        if news_clip and model:
            with st.spinner("解析中..."):
                res = model.generate_content(f"請以教育行政與校長甄試視角，分析以下文本的關鍵考點、政策背景及實務建議：\n{news_clip}")
...
with tab2:
    ...
    with col_n1:
        note_t = st.text_input("📌 專題名稱：", placeholder="例如：113年校事會議處理程序...")
    with col_n2:
    ...
    if st.button("📖 生成行政戰略架構"):
        if model and note_t:
            with st.spinner("整理中..."):
                p = f"主題：{note_t}\n參考法規：{ref_text_note}\n請撰寫包含前言、內涵、策略KPI表格、結語的精簡筆記。"
                st.markdown(model.generate_content(p).text)

# --- Tab 3: 實戰模擬 (雙向校準邏輯完全保留) ---
with tab3:
    st.markdown("""
    <div class="alert-box">
    🎯 <strong>校準機制說明：</strong> 若要練習最新的「校事會議」或「霸凌新制」，請務必在下方「法規校準座」貼入條文。
    AI 將以此文本為 <strong>唯一評分真理</strong>，修正其內建的舊知識。
    </div>
    """, unsafe_allow_html=True)
    ...
    with c3:
        ...
        manual_theme = st.text_input("主題", placeholder="若不填則依向度命題...", label_visibility="collapsed", key="custom_theme_tab3")
    with c4:
    ...
    with st.expander("⚖️ 法規校準座 (貼入最新條文以校準 AI 閱卷標準)"):
        ref_text_sim = st.text_area("在此貼上最新的法規條文或 SOP...", height=150, key="sim_ref_input")

    # 3. 命題邏輯
    if gen_btn and model:
        with st.spinner("正在校準並命題中..."):
            target = manual_theme if manual_theme.strip() else THEME_POOL[sel_choice]
            q_prompt = f"你是校長甄試命題委員。請針對『{target}』設計一題實務申論題。參考法規：{ref_text_sim}。要求：包含情境與三項任務，直接輸出題目。"
    ...
    # 5. 作答與評分
    st.markdown('<p class="tiny-label">🖋️ 擬答作答區 (模擬真實申論卷)</p>', unsafe_allow_html=True)
    ans_input = st.text_area("作答內容", label_visibility="collapsed", key="v11_ans_sim_final")

    col_info, col_submit = st.columns([1, 1])
    with col_info:
        st.markdown(f'<span class="word-count-badge">📝 目前字數：{len(ans_input)}</span>', unsafe_allow_html=True)
    with col_submit:
        if st.button("⚖️ 提交閱卷評分 (依據校準文本)", use_container_width=True):
            if model and ans_input:
                with st.spinner("閱卷委員評分中..."):
                    eval_prompt = f"""
                    請擔任嚴謹的閱卷委員評分：
                    題目：{st.session_state.current_q}
                    唯一正確法規參考：{ref_text_sim}
                    考生擬答：{ans_input}
                    ---
                    指令：若擬答與『參考法規』不符，必須扣分並明確指正。滿分25分。給予優點、缺點、具體修正建議。
                    """
