...
with tab2:
    ...
    if st.button("📖 生成精確策略矩陣"):
        if model:
            with st.spinner("戰略煉製中..."):
                # 修改後的 Prompt：去除演講感，強調精確度與桃園連結
                p = f"""
                你現在是專業教育行政導師。請針對專題『{note_t}』，以「去頭去尾、直擊精華」的方式提供實務戰略。
                
                【限制要求】：
                1. 嚴禁任何演講式開場白（例如：各位老師、身為校長我認為...）。
                2. 嚴禁口語化的發言論述。
                3. 直接輸出結構化內容，使用標題與表格。
                
                【核心輸出架構】：
                - **戰略核心 (Why)**：本專題的核心價值與必要性。
                - **行動矩陣 (Who, What, How)**：
                    - Who：具體對應的利害關係人（行政、教師、學生、家長）。
                    - What：核心推動目標。
                    - How：最具體的執行策略（請分為 3-4 個關鍵行動點）。
                - **桃園政策連結**：必須精確對接桃園市「教育善好」政策（例如：數位轉型、在地國際化、品格教育等具體計畫）。
                - **關鍵績效指標 (KPI)**：提供 3 個可觀察、可量化的具體指標。
                """
