...
# 1. 頁面基本設定 (必須在第一行)
st.set_page_config(page_title="體育課程研究室 - 終極定稿版", layout="wide", page_icon="🏫")

# --- 🎨 核心 CSS：穩定結構、固定高度、香檳杏色調 ---
st.markdown("""
    <style>
...
# --- 2. 核心 AI 初始化 (🛠️ 修復路徑錯誤) ---
@st.cache_resource
def init_ai():
    try:
        genai.configure(api_key=st.secrets["gemini"]["api_key"])
        # 修正：必須使用 models/ 前綴以避免 NotFound 錯誤
        return genai.GenerativeModel("models/gemini-1.5-flash")
    except Exception as e:
        st.error(f"AI 啟動異常: {e}")
        return None
...
THEME_POOL = {
    ...
    "⚖️ 法理實務與危機處理": "校事會議、霸凌性平新制、親師衝突管理、組織正義。",
...
with tab1:
    ...
    if st.button("🎯 執行深度解析"):
        if news_clip and model:
            with st.spinner("解析中..."):
                res = model.generate_content(f"請分析此文本之核心要義與考點：\n{news_clip}")
                st.info(res.text)
...
with tab2:
    ...
    if st.button("📖 生成戰略矩陣"):
        if model and note_t:
            with st.spinner("煉製中..."):
                res = model.generate_content(f"針對專題『{note_t}』，提供學理、行動矩陣與KPI。")
        ...
        if st.button("生成試題", use_container_width=True):
            if model:
                with st.spinner("擬仿真題命題中..."):
                    ...
                    q_prompt = f"效法「第 29 期試題」風格命題。主題：『{target_topic}』。情境精煉(100-150字)、融合學理、任務明確。"
...
# --- Tab 4: 歷程紀錄 (完整功能復原) ---
with tab4:
    st.markdown("### 📊 學習歷程數據分析")
    df = get_records()
    if not df.empty:
        df['score_num'] = pd.to_numeric(df['實戰分數'], errors='coerce')
        m1, m2 = st.columns(2)
        with m1: st.metric("平均得分", f"{df['score_num'].mean():.1f}")
        with m2: st.metric("最高得分", f"{df['score_num'].max():.0f}")
