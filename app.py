    ...
    if st.button("📖 生成精確策略矩陣"):
        if model:
            with st.spinner("戰略核心與內涵煉製中..."):
                ...
                【核心輸出架構】：
                - **戰略核心 (Why) — 理念與面向**：
                    1. **核心理念 (Philosophy)**：從教育本質、學生主體或未來趨勢出發，用具備行政厚度的一段話說明本案推動的核心價值。
                    2. **核心面向 (Dimensions/Connotations)**：將此專題拆解為具體的推動內涵（例如：制度建構面、專業發展面、環境營造面、資源整合面等關鍵面向）。
                - **行動矩陣 (Who, What, How)**：
                    - Who：具體對應的利害關係人分工。
                    - What：核心達成目標。
                    - How：最具體的執行策略行動點（3-4 個關鍵作法）。
                - **桃園政策連結**：精確對接桃園市「教育善好」政策。
