...
# --- 2. 核心 AI 初始化 (🛠️ 絕對路徑修正) ---
@st.cache_resource
def init_ai():
    try:
        genai.configure(api_key=st.secrets["gemini"]["api_key"])
        # 修復：在 v1beta 報錯時，最穩定的寫法是直接寫模型名，不帶 prefix 或帶正確 prefix
        # 這裡鎖定最標準的寫法
...
THEME_POOL = {
    ...
    "📖 教學領航與數位轉型": "GenAI應用、數位公民素養、PLC專業學習社群。",
    "⚖️ 法理實務與危機處理": "校事會議、霸凌性平新制、親師衝突管理。",
...
with tab2:
    ...
    note_t = st.text_input("輸入專題名稱：", placeholder="例如：桃園教育願景下之韌性領導", key="note_input")
    if st.button("📖 生成戰略矩陣"):
        if model and note_t:
            with st.spinner("煉製中..."):
...
with tab3:
    ...
    with c_input:
        ...
        # 固定手動輸入框，絕對不刪除
        manual_theme = st.text_input("自訂主題", placeholder="若不填則依向度命題...", key="manual_t_fix", label_visibility="collapsed")
        ...
        if st.button("生成試題", use_container_width=True):
            if model:
                with st.spinner("模擬命題中..."):
                    ...
                    q_prompt = f"你是校長甄試委員。請效法第29期風格，針對『{target_topic}』命一題申論題。情境150字內，融合1個教育行政學理，任務明確。直接輸出題目。"
    ...
    with col_a:
        ...
        ans_input = st.text_area("作答區", label_visibility="collapsed", key="ans_box_final_v3", height=520)
    ...
    if 'feedback' in st.session_state:
        st.markdown(f"<div style='margin-top:30px; padding:25px; background:#2d323e; border-radius:18px; border-left:6px solid #d4c19c; color:#e2e8f0;'>{st.session_state.feedback}</div>", unsafe_allow_html=True)
...
# 1. 頁面基本設定 (第一行)
st.set_page_config(page_title="體育課程研究室 - 穩定全功能版", layout="wide", page_icon="🏫")

# --- 🎨 核心 CSS：鎖定 520px 高度、香檳杏視覺 ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@300;400;500;700&display=swap');
...
def init_ai():
    try:
        genai.configure(api_key=st.secrets["gemini"]["api_key"])
        # 修正：直接使用 gemini-1.5-flash 避免 NotFound
        return genai.GenerativeModel("gemini-1.5-flash")
    except Exception as e:
...
# --- 3. 向度池 ---
THEME_POOL = {
    "🏆 領導願景與品牌經營": "桃園教育願景、品牌學校、ESG、校長領導各種學術理論。",
    "📘 課程發展與課綱領航": "108課綱深綱、雙語與SDGs、跨域課程整合。",
...
with tab1:
    ...
    if st.button("🎯 執行深度解析"):
        if news_clip and model:
            with st.spinner("解析中..."):
                res = model.generate_content(f"請分析此文本之核心要義、校長對策與考點：\n{news_clip}")
                st.info(res.text)
...
with tab3:
    ...
    with c_timer_btn:
        st.markdown('<p class="tiny-label">⏱️ 計時器</p>', unsafe_allow_html=True)
        if st.button("啟動練習", use_container_width=True):
            st.session_state.start_time = time.time()
            st.session_state.timer_running = True
    ...
    with c_input:
        st.markdown('<p class="tiny-label">🖋️ 手動輸入自訂主題</p>', unsafe_allow_html=True)
        # 確保手動輸入框存在
        manual_theme = st.text_input("自訂主題", placeholder="若不填則由向度出題...", key="custom_t_final_v2", label_visibility="collapsed")
        ...
        if st.button("生成試題", use_container_width=True):
            if model:
                with st.spinner("模擬命題委員出題中..."):
                    ...
                    q_prompt = f"請擔任「校長甄試命題委員」。效法「第 29 期試題」風格，針對主題『{target}』命題。情境100-150字，結合學理，任務明確，嚴禁贅詞。"
                    try:
    ...
    with col_a:
        st.markdown('<p class="tiny-label">🖋️ 擬答作答區</p>', unsafe_allow_html=True)
        ans_input = st.text_area("作答區", label_visibility="collapsed", key="ans_box_stable", height=520)
        f_count, f_submit = st.columns([1, 1])
        with f_count: st.markdown(f'<span class="word-count-badge">📝 當前字數：{len(ans_input)}</span>', unsafe_allow_html=True)
        with f_submit:
            if st.button("⚖️ 提交閱卷評分", use_container_width=True):
...
with tab4:
    ...
    if not df.empty:
        ...
        # 詳細表格
        st.markdown('<p class="tiny-label">📋 詳細練習紀錄</p>', unsafe_allow_html=True)
        st.dataframe(df[['紀錄時間', '題目主題', '實戰分數', '我的作答', 'AI 評語摘要']], use_container_width=True)
    else:
        st.info("尚無練習紀錄，請前往「實戰模擬」開始第一次挑戰！")
